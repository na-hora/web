...
import { CopyOutlined, MailOutlined, UserOutlined, WhatsAppOutlined } from '@ant-design/icons'
  ...
  return (
    ...
    >
        ...
        <Col style={{ margin: '40px 0' }}>
          <p><UserOutlined style={{ marginRight: '4px' }} /><strong>Cliente:</strong> {selectedAppointment?.client?.name}</p>
          ...
          <p>
            ...
            <Tooltip title={copyPhoneStatus ? 'Copiado!' : 'Copiar'}>
              <CopyOutlined
                ...
                onClick={() => {
                  navigator.clipboard.writeText(selectedAppointment?.client?.phone);
          ...
          <p>
            ...
            <Tooltip title={copyEmailStatus ? 'Copiado!' : 'Copiar'}>
              <CopyOutlined
                style={{ marginLeft: '8px', cursor: 'pointer' }}
                onClick={() => {
                  navigator.clipboard.writeText(selectedAppointment?.client?.email);
          ...
          <p><strong>Serviço:</strong> {selectedAppointment?.serviceName}</p>
          <p><strong>Horário Inicial:</strong> {formatDateTime(selectedAppointment?.start)}</p>
          <p><strong>Horário Final:</strong> {formatDateTime(selectedAppointment?.end)}</p>
        </Col>
    </Modal>
  )
}
